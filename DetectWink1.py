...
def detectWink(frame, location, ROI, cascade):
    eyes = cascade.detectMultiScale(
        ROI, 1.27 ,7 , 0|cv2.CASCADE_SCALE_IMAGE, (10, 20))

    if len(eyes) is 0:
        eyes = cascade.detectMultiScale(ROI, 1.02 ,4 ,0|cv2.CASCADE_SCALE_IMAGE, (10, 10))
...
def detect(frame, faceCascade, eyesCascade, i):
    ...
    for f in faces:
        ...
        if detectWink(frame, (x, y), faceROI, eyesCascade):
            detected += 1
            #print(i, "Winks detected", detected)
...
def run_on_folder(cascade1, cascade2, folder):
    ...
    for f in files:
        ...
        if type(img) is np.ndarray:
            i += 1
            lCnt = detect(img, cascade1, cascade2, i)
            totalCount += lCnt
            if windowName != None:
